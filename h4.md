...
- I used the default configuration file, uncommented a few settings plus added the ports `222` and `2222` for sshd to listen to.
- `$ sudo cp /etc/ssh/sshd_config /srv/salt/ssh/files/sshd_config` copies the default file to our directory.
...
- Picture 1: The openssh-server was already installed (succeeded 1)
  - ![Screenshot 2025-04-22 at 12 38 25](https://github.com/user-attachments/assets/fb7340df-cd3d-48d5-8026-f6cd1c05d0e8)
- Picture 2: The configuration file was updated with our latest configurations (succeeded 1, changed=1)
  - ![Screenshot 2025-04-22 at 12 38 55](https://github.com/user-attachments/assets/0234b139-7fe0-4870-80a1-d1bd7550314b)
- Picture 3: The configurations were applied, and the service was restarted, now being enabled and running (succeeded 1, changed=1)
  - ![Screenshot 2025-04-22 at 12 39 27](https://github.com/user-attachments/assets/57b6cedc-1c59-4a21-bf87-21a133c9079b)
- Picture 4 (quick math): Confirmation of what I just said.
  - ![Screenshot 2025-04-22 at 12 39 42](https://github.com/user-attachments/assets/0f7bb0a0-b6ad-4d2b-a663-48d576e7ab8f)


### But did it work?
Trying out the connection on port 222 by scanning it using the netcat tool.
...
To my amazement it did not work... So I tried to do the same on the default port just to be sure that the tool I was using was working properly. 
  -  ![Screenshot 2025-04-22 at 12 47 52](https://github.com/user-attachments/assets/812e4c82-48ed-4148-8605-be99ab5a1720)

### Troubleshooting
Apparently the problem was not in the tool I was using. I instantly navigated to my minion for some troubleshooting `$ exit` --> `$ vagrant ssh minion` --> `$ less /etc/ssh/sshd_config`
...
- I noticed that when the service restarted, it didn't include the other ports correctly.
- So I went ahead and restarted the service manually with `$ sudo systemctl restart ssh`
- This still didn't help, so I used the ol' reliable `$ sudo reboot` which totally took care of all my problems 😂
