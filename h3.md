...
- First line of bussines was establishing the relationship between the master and the slaves:
  ...
  - Making sure the connection works by the help of `$ sudo salt '*' test.ping`
    - ![Screenshot 2025-04-14 at 15 17 20](https://github.com/user-attachments/assets/96856c01-d0db-43dc-a4ca-f97285ae6ced)

**Running your own module**
- We need the root directory so that Salt can check for states, and at the same time, we want to create a directory for our own module. Easy: `$ sudo mkdir -p /srv/salt/module1`. This command creates the required root `/srv/salt` (if it doesn't already exist) and sets up the base directory for our first module
- `$ sudo nano init.sls` will create the default file that will be ran when we call our module. 
- By running my init file, it will lead to the creation of a file called "emptyfile.txt" in the `/tmp` director if it doesn't already exist.
...
- Lets's add some text to the empty file!
  ...
  - By using the state function `file.append` we were successfully able to append some text to the file we created earlier. (duplicate output retracted, just take my word for it that it worked on both minons).
  ...
  - **A:** Clue number #1 = the comment already tells us that "file is in correct state", and clue number #2, we make the observatin, that this time around we have no `changed=1`, but only the `Succeeded: 1`.
...
- **Installing packages**
  - `$ sudo salt '*' state.apply module1.tools`
    ...
    - We can see that `tmux` was already installed, but apache and tree were yet to be downloaded (or updated).
  ...
  - Making sure everything went as expected
    - ![Screenshot 2025-04-14 at 17 05 30](https://github.com/user-attachments/assets/9c641578-5a2b-4dbf-bc7e-457dad5fb690)
- **One file to rule them all?**
  - All the setting we have been applying, could obviusly have been typed in one single file, but this was just for educational purposes. It's easier to hang on this way.
  - Now we're going to make things super easy for us. Let's just create one single file, and then apply the state to all our minions with one command: `$ sudo salt '*' state.apply module1`
  ...
  - Q: Is our init.sls file still idempotent?
  - A: Very much is thank you for asking! 🙏
