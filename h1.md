...
- Without the argument `creates`, it will just _overwrite_ an existing file/directory, so initself the command is not idempotent. But with the argument `creates` and the variable, it will only create it if the variable is false. So in this case because we already have a directory called playingGround (=true), it will not try to create it again and overwrite the whole thing. We achieved idempotency in this case!
...
> [!WARNING]
> In this example I used a variable that will always be false unless i go manually creating the file now
>
> You **shouldn't** use different arguments within the command like shown here. Always use the same specs for the command and the
> `creates`
> 
