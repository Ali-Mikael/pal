...
- The `$minion` script, as the name suggests would install salt-minion. It would then configure the masters IP, minion hostname, and then reload the service so that the new configurations were to step in place.
...
- This file was placed in a directory
...
- The idea was to accept the Minion-keys straight up. But here was the problem I mentioned before. Because I forgot to configure the service (salt-master) to enable and start after installation, the minions couldn't connect to it.
  ...
  - `$ sudo salt-key -L` shoudl list the keys, but I didn't see any, which initially sparked my worry.
